...
- **`src/types/index.ts`** — Exports `Dictionary` type (derived from `en.json` shape) plus per-section subtypes (`NavDict`, `HeroDict`, `ServicesDict`, `AboutMeDict`, `StatusDict`, `EngineDict`, `ClipperNetworkDict`, `WorkflowDict`, `FAQDict`, `CTADict`, `FooterDict`, `LegalPageDict`, etc.).

### Page Structure

`src/app/[locale]/page.tsx` assembles 8 section components + StatusIndicator in order:
1. **StatusIndicator** — Fixed-position "Live Status" badge (top-right)
2. **HeroSection** — Massive heading with word-by-word reveal animation, CSS/SVG visual, glassmorphic stat box
3. **EngineSection** — Horizontal-to-vertical content transformation flow with SVG animated dots
4. **ServicesSection** (`#services`) — 5-card asymmetric bento grid with custom CSS/SVG visuals
5. **ClipperNetworkSection** (`#clipper-network`) — 4 clipper profile cards with staggered layout
6. **WorkflowSection** (`#how-it-works`) — Horizontal scrollable 4-step process
7. **AboutSection** (`#founders`) — GitHub-style 2-founder cards
8. **FAQSection** (`#faq`) — Accordion with click ripple effect
9. **CTASection** (`#contact`) — "Viral Loop?" with glitch effect, lime CTA
...
- **`components/ui/`** — Atomic components: `Button` (variants: `primary`/`secondary`/`lime`/`surface`/`ghost`, all uppercase Space Grotesk), `Container` (max-w-7xl), `SectionHeading` (text-5xl font-black uppercase tracking-tighter), `CalComLogo`, `StatusIndicator` (fixed lime pulsing badge, hidden on mobile).
...
- HeroSection uses word-by-word `motion.span` reveal with clipPath stagger (0.15s per word). Right column has abstract CSS/SVG visual with glassmorphic stat box.
...
- ClipperNetworkSection has 4 cards in staggered grid (cards 2/4 offset by `md:mt-12`), CSS gradient avatars with `grayscale group-hover:grayscale-0`, CountUp views, animated progress bars.
...
- AboutSection has 2 founder cards with GitHub avatars, lime GitHub button, lime status dot.
- FAQSection uses AnimatePresence accordion with click ripple effect and `secondary` left accent on hover.
